...
def get_food_order_by_limit(db: Session, user_id: str) -> List[str]:
    food_names = db.query(Food.name).filter(
        Garage.user_id == user_id, Food.food_id == Garage.food_id).order_by(Garage.limit_at).limit(3).all()
    foods = []
